 ...
 * @since 1.1.0
 */
class Cholot_TextIcon extends Widget_Base
{
	...
	{
		return __('Cholot Text Icon', 'cholot_plugin');
	}
	...
	protected function _register_controls()
	{
		...
		$this->add_control(
			'icon_style',
			[
				'label' => __('Text & Icon  Type', 'cholot_plugin'),
				'type' => Controls_Manager::SELECT,
				'options' => [
					'center' => __('Centered Icon', 'cholot_plugin'),
					'left' => __('Left Aligned Icon', 'cholot_plugin'),
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .cholot-icon' => 'font-size: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .cholot-icon' => 'width: {{SIZE}}{{UNIT}};height: {{SIZE}}{{UNIT}};line-height: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .cholot-icon' => 'line-height: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => ['px', '%'],
				'selectors' => [
					'{{WRAPPER}} .cholot-icon' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .cholot-icon' => 'margin-top: {{TOP}}{{UNIT}}; margin-bottom: {{BOTTOM}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .cholot-icon' => 'margin:{{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .cholot-icon' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .cholot-icon' => 'background-color: {{VALUE}};',
				],
			]
		);
...
<div class="<?php if ($settings['icon_style'] != 'left') {
					echo 'box-small-icon';
				} else {
					echo 'box-with-icon';
				} ?>">
    <?php if (!empty($settings['icon']) || !empty($settings['selected_icon']['value'])) :  if ($is_new || $migrated) :
				Icons_Manager::render_icon($settings['selected_icon'], ['class' => 'cholot-icon']);
			else : ?>
    <i class="cholot-icon <?php echo esc_attr($settings['icon']); ?>" aria-hidden="true"></i>
    <?php endif;
		endif; ?>
